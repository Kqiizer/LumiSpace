...
if (function_exists('actualizarCantidadCarrito')) {
  $ok = actualizarCantidadCarrito($usuario_id, $producto_id, $cantidad);
} else {
  $ok = false;
}

echo json_encode(['ok' => $ok, 'msg' => $ok ? 'Cantidad actualizada' : 'Error al actualizar']);
